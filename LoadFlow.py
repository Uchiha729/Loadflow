...
class Line:
    ...
    def __init__(self, fromNode: Node, toNode: Node, r: float, x: float, b_half: float, name=None):
        Line.lineNumber += 1
...
class Grid:
    ...
    def NR(self, tol=1, maxIter=1000, BMva=230):
        self.iter = 0
        Pg = self.Pg / BMva
        Qg = self.Qg / BMva
        Pl = self.Pl / BMva
    ...
    def printResults(self):
        ...
        for i in range(self.nb):
            print('| %3g | %8.3f | %8.5f |' % (i+1, self.nodes[i].vLf, self.nodes[i].thetaLf * 180 / np.pi))
        print()